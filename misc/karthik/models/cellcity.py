...
from city.cellsector import Sector
from city.road import SimpleRoad as road
...
class CellularAutomataCity(BaseGenerator):
    ...
    def generate(self, config):
        ...
        group_length = lambda g: ((g.max_col - g.min_col + 1) * cell_size)\
                                                                    - cell_gap
        group_width = lambda g: ((g.max_row - g.min_row + 1) * cell_size)\
                                                                    - cell_gap
...
#            self.add_geom(plate)
            ...
            x, y, z = origin
            left_road = (x - cell_gap, y, z)
            right_road = (x + length, y, z)
            v_road_origins = [left_road, right_road]
            bottom_road = ((x, y, z), (-math.pi / 2, (0, 0, 1)))
            up_road = ((x, y + width + cell_gap, z), (-math.pi / 2, (0, 0, 1)))
...
#                lake_id = self.get_material('lake', M(texture='lake.jpg'))
#                forest_id = self.get_material('forest', M(texture='forest.jpg'))
#                x = 1000
#                if g.type == Cell.FOREST:
#                    box.set_material(forest_id, texture_coords=[(0.0, 0.0), (x, 0.0), (x, x), (0.0, x)])
#                else:
#                    box.set_material(lake_id, texture_coords=[(0.0, 0.0), (x, 0.0), (x, x), (0.0, x)])
#
#                x, y, z = origin
...
#        road_id = self.get_material('road', M(texture='road.jpg'))
#        x = 10000
#        plate.set_material(road_id, texture_coords=[(0.0, 0.0), (x, 0.0), (x, x), (0.0, x)])
...
class Group:
    ...
    def grow_in_direction(self, direction):
        ...
        if self.habitable:
            ...
            max_area = c.layout_max_cells[self.level - 1]
